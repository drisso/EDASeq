...
\newcommand{\EDASeq}{\Rpackage{EDASeq}}

\newcommand{\Bowtie}{\software{Bowtie} }


\title{\Rpackage{EDASeq}: Exploratory Data Analysis and Normalization for RNA-Seq}
\author{Davide Risso}
\date{Modified: May 22, 2012.  Compiled: \today}
\begin{document}
...
To illustrate the functionality of the \Rpackage{EDASeq} package, we
...
The \Rpackage{leeBamViews} package provides a subset of the aligned
...
The \Rpackage{yeastRNASeq} package contains gene-level read counts for
...
the class \Rclass{FastqFileList} imported from \Rpackage{ShortRead}.
...
the class \Rclass{BamFileList} from \Rpackage{Rsamtools}.  Again, the
\Rcode{elementMetadata} slot can be used to store lane-level sample
information.

<<import-aligned,results=verbatim>>=
...
<<plot-total,fig=TRUE,include=FALSE>>=
...
Figure \ref{fig1}, produced using the \Rcode{barplot} method for the
\Rclass{BamFileList} class, displays the number of mapped reads for
the subset of the yeast dataset included in the package
\Rpackage{leeBamViews}.  Unfortunately, \Rpackage{leeBamViews} does
...
\Rclass{FastqFileList} class. Analogously, one can plot the percentage
...
\begin{figure}
  \centering
  \subfigure[Number of mapped reads]{
  \includegraphics[width=.45\textwidth]{EDASeq-plot-total}
    \label{fig1}
  }
  \subfigure[Mean per-base quality of mapped reads]{
    \includegraphics[width=.45\textwidth]{EDASeq-plot-mean-qual}
    \label{fig2}
   }   
   \caption{Per-lane number of mapped reads and quality scores.}
\end{figure}
...
<<plot-mean-qual,fig=TRUE,include=FALSE>>=
...
each lane (Figure \ref{fig5}) and the number of mapped reads
stratified by chromosome (Figure \ref{fig6}) or strand. As expected,
all the reads are mapped to chromosome XIII.

<<plot-qual-1,fig=TRUE,include=FALSE>>=
plotQuality(bfs[[1]],cex.axis=.8)
@ 

<<plot-mapped-1,fig=TRUE,include=FALSE>>=
barplot(bfs[[1]],las=2)
@ 

\begin{figure}
  \centering
  \subfigure[Per-base quality of mapped reads]{
    \includegraphics[width=.45\textwidth]{EDASeq-plot-qual-1}
    \label{fig5}
  } 
  \subfigure[Number of mapped reads per-chromosome]{
    \includegraphics[width=.45\textwidth]{EDASeq-plot-mapped-1}
    \label{fig6}
  }
  \caption{Quality scores and number of mapped reads for lane ``isowt5\_13e''.}
\end{figure}
...
lane (Figure \ref{fig7}).

<<plot-nt-1,fig=TRUE,include=FALSE>>=
plotNtFrequency(bfs[[1]])
@ 

\begin{figure}
  \centering
  \includegraphics[width=.55\textwidth]{EDASeq-plot-nt-1}
  \caption{Per-base nucleotide frequencies of mapped reads for lane ``isowt5\_13e''.}
  \label{fig7}   
\end{figure}
...
a particular genome annotation, e.g., \Rpackage{IRanges},
\Rpackage{ShortRead}, \Rpackage{Genominator}, \Rpackage{Rsubread}. See
their respective vignettes for details.

Here, we consider this step done and load the object
\Robject{geneLevelData} from \Rpackage{yeastRNASeq}, which provides
gene-level counts for 2 wild-type and 2 mutant lanes from the yeast
dataset of \citet{lee2008novel} (see the \Rpackage{Genominator}
...
the \Rpackage{EDASeq} package provides, for illustration purposes,
...
The \Rpackage{EDASeq} package provides the \Rclass{SeqExpressionSet}
...
Note that the row names of \Robject{exprs} and \Robject{featureData}
must be the same; likewise for the row names of \Robject{phenoData}
and the column names of \Robject{exprs}.  As in the
\Rclass{ExpressionSet} class, the expression values can be accessed
with \Rfunction{exprs}, the lane information with \Rfunction{pData},
...
The \Rclass{SeqExpressionSet} class has an additional slot called
\Robject{offset} (matrix of the same dimension as \Robject{exprs}),
which may be used to store a normalization offset to be supplied to a
model for read counts in differential expression analysis (see Section
\ref{secDE} and the vignette for \Rpackage{edgeR} for details on the
role offsets). If not specified, the offset is initialized as a matrix
of zeros.
...
\Rmethod{boxplot} method provides an easy way to produce boxplots of
the logarithms of the gene counts in each lane (Figure \ref{fig8}).

<<boxplot-genelevel,fig=TRUE,include=FALSE>>=
boxplot(data,col=colors[1:4])
@ 

The \Rmethod{MDPlot} method produces a mean-difference plot (MD-plot)
of read counts for two lanes (Figure \ref{fig8b}).

<<md-plot,fig=TRUE,include=FALSE>>=
MDPlot(data,c(1,3))
@ 
...
\begin{figure}
  \centering
  \includegraphics[width=.45\textwidth]{EDASeq-boxplot-genelevel}
  \caption{Between-lane distribution of gene-level counts (log).}
  \label{fig8}   
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=.45\textwidth]{EDASeq-md-plot}
  \caption{Mean-difference plot of the gene-level counts (log) of
    lanes ``mut\_1'' and ``wt\_1''.}
  \label{fig8b}   
\end{figure}
...
expect the points in Figures \ref{fig9a} and \ref{fig9b} to be evenly
scattered around the black line).

<<plot-mean-var-mut,fig=TRUE,include=FALSE>>=
meanVarPlot(data[,1:2],log=T, ylim=c(0,16))
@ 

<<plot-mean-var-all,fig=TRUE,include=FALSE>>=
meanVarPlot(data,log=T, ylim=c(0,16))
@ 
...
mutant technical replicate lanes (Figure \ref{fig9a}); likewise for
the two wild-type lanes. However, one expects over-dispersion in the
presence of biological variability, as seen in Figure \ref{fig9b} when
considering at once all four mutant and wild-type lanes
\citep{anders2010differential,bullard2010evaluation,robinson2010edger}.


\begin{figure}
  \centering
  \subfigure[Mutant lanes]{
    \includegraphics[width=.45\textwidth]{EDASeq-plot-mean-var-mut}
    \label{fig9a}
  }
  \subfigure[All four lanes]{
    \includegraphics[width=.45\textwidth]{EDASeq-plot-mean-var-all}
    \label{fig9b}
  }
  \caption{Mean-variance relationship for the two mutant lanes and all
    four lanes: the black line corresponds to the Poisson distribution
    (variance equal to the mean), while the red curve is a lowess fit.}
\end{figure}
...
In Figure \ref{fig10}, obtained using \Rfunction{biasPlot}, one can
...
<<plot-gc,fig=TRUE,include=FALSE>>=
biasPlot(data,"gc",log=T,ylim=c(1,5))
@ 

\begin{figure}
  \centering
  \includegraphics[width=.55\textwidth]{EDASeq-plot-gc}
  \caption{Lowess regression of the gene-level counts (log) on 
    GC-content for each lane, color-coded by experimental condition.}
  \label{fig10}   
\end{figure}
...
of read counts from two lanes using the \Rmethod{biasBoxplot} method
(Figure \ref{fig10b}). Again, the same type of plots can be created
for gene length or mappability.

<<boxplot-gc,fig=TRUE,include=FALSE>>=
...
\begin{figure}
  \centering
  \includegraphics[width=.55\textwidth]{EDASeq-boxplot-gc}
  \caption{Boxplots of the log-fold-change between ``mut\_1'' and
    ``wt\_1'' lanes stratified by GC-content.}
  \label{fig10b}   
\end{figure}
...
Figure \ref{fig11} shows how after full-quantile within- and
between-lane normalization, the GC-content bias is reduced and the
distribution of the counts is the same in each lane.

<<plot-gc-norm,fig=TRUE,include=FALSE>>=
biasPlot(dataNorm,"gc",log=T,ylim=c(1,5))
@ 

<<boxplot-norm,fig=TRUE,include=FALSE>>=
boxplot(dataNorm,col=colors)
@ 

\begin{figure}
  \centering
  \subfigure[GC-content]{
    \includegraphics[width=.45\textwidth]{EDASeq-plot-gc-norm}
 }
  \subfigure[Count distribution]{
    \includegraphics[width=.45\textwidth]{EDASeq-boxplot-norm}
 }
  \caption{Full-quantile within- and between-lane
normalization. (a) Lowess regression of normalized gene-level counts
(log) on GC-content for each lane. (b) Between-lane distribution of normalized gene-level counts (log).}
  \label{fig11}
\end{figure}
...
offset) obtained using the \Rpackage{EDASeq} package can be supplied
to packages such as \Rpackage{edgeR} \citep{robinson2010edger} or
\Rpackage{DESeq} \citep{anders2010differential} to find differentially
expressed genes. This section should be considered only as an
illustration of the compatibility of the results of \Rpackage{EDASeq}
...
\Rpackage{edgeR} based on the original counts by passing an offset to
the generalized linear model. For simplicity, we estimate a common dispersion
parameter for all genes. See the \Rpackage{edgeR} vignette for details
...
fit <- glmFit(exprs(dataOffset), design, disp, offset=-offst(dataOffset))
...
We can perform a differential expression analysis with
\Rpackage{DESeq} based on the normalized counts by using the coerce
method from the \Rclass{SeqExpressionSet} class to the
\Rclass{CountDataSet} class of \Rpackage{DESeq}.  When working with
...
<<unrounded>>=
dataNorm <- betweenLaneNormalization(data, round=FALSE)
dataOffset <- betweenLaneNormalization(data, offset=TRUE)

norm1 <- exprs(dataNorm)
norm2 <- exp(log(exprs(dataOffset) + 0.1 ) + offst(dataOffset)) - 0.1
...
<<rounded>>= 
head(round(exprs(dataNorm)) - round(exprs(dataOffset) * exp(offst(dataOffset))))
@ 

We defined the offset as the log-ratio between normalized and raw
counts. However, the \Rpackage{edgeR} functions expect as offset
argument the log-ratio between raw and normalized counts. One must use
\texttt{-offst(offsetData)} as the offset argument of \Rpackage{edgeR}
