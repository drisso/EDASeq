...
\author{Davide Risso}
\date{Modified: October 7, 2011.  Compiled: \today}
\begin{document}
...
strain, there are two technical replicate lanes from the same library
...
four lanes: the two replicates of the wild-type strain (``wt'') and
the two replicates of one of the mutant strains (``mut'').  We use
...
of reads produced in each lane, the number of reads mapped to a
reference genome, and the percentage of mapped reads. A low total
...
reads (low complexity) or problems with the reference genome.
...
\Rclass{BamFileList}, \Rclass{FastqFileList}. See the manual pages for
details.
...
the generalized linear model. Here, we estimate a common dispersion
parameter for all genes.
